  ...
  beforeEach(async () => {
    ...
    wrapper = render(
      <UserProvider value={testUser}>
        <Routes>
          <Route element={<UserItem />} path={userItemsItem()} />
          <Route element={<div data-testid="item-page" />} path={item()} />
          <Route
      ...
      { router: { initialEntries: [userItemsItem()] } },
    );
    await waitFor(() => expect(itemGet).toHaveBeenCalledTimes(1));
    await waitFor(() =>
      expect(
        wrapper.queryByTestId('user-items__item__error--loading'),
      ).toBeInTheDocument(),
    );
  });
  ...
  describe('When retrying and the API call succeeds', () => {
    ...
    describe('When the user does not own this item', () => {
      beforeEach(async () => {
        (itemGet as jest.Mock)
          .mockClear()
          .mockResolvedValue({ data: mockItem, status: 200 });
        fireEvent.click(
          wrapper.getByTestId('user-items__item__error--loading'),
        );
        await waitFor(() => expect(itemGet).toHaveBeenCalledTimes(1));
        await waitFor(() =>
    ...
    describe('When the user owns this item', () => {
      beforeEach(async () => {
        (itemGet as jest.Mock).mockClear().mockResolvedValue({
          data: itemMock({ ...mockItem, user: testUser }),
          status: 200,
        });
