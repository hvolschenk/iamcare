  ...
  describe('After uploading some images', () => {
    ...
    describe('After removing some uploaded images', () => {
      beforeEach(() => {
        onChange.mockClear();
        fireEvent.click(
          wrapper.getByTestId('file-upload__uploaded-item__remove--3'),
        );
        fireEvent.click(
          wrapper.getByTestId('file-upload__uploaded-item__remove--2'),
        );
      });

      test('Calls the change handler for each file removed', () => {
        expect(onChange.mock.calls[1][0]).toEqual([image1, image2]);
      });
    });
  });
});
